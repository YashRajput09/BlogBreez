...
const MyProfile = () => {
  ...
  return (
    <div className='w-full h-screen flex justify-center items-center flex-wrap py-4 gap-5 overflow-hidden'>
      <div key={creator._id} className='m-2 flex flex-col creators-center rounded-lg shadow-lg overflow-hidden group'>
      ...
      <Link to={`/user/update/admin/profile/${creator._id}`} className='border-2 rounded-lg py-1 m-2 bg-blue-400 hover:bg-blue-500  duration-300 ease-in-out text-white text-center'>Update</Link>
    </div>
    </div>
  );
};
